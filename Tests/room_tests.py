...
class TestRoomCreation(unittest.TestCase):
    ...
    def test_for_maximum_living_space_created_are_4(self):
        self.dojo.create_room('living', ['Kenya', 'Uganda', 'Tanzania', 'Rwanda', 'somalia'])
        self.assertTrue(len(self.dojo.living_spaces) == 4, 'Cannot create more than 4 living spaces')

    def test_for_maximum_offices_created_are_six(self):
        self.dojo.create_room('office', ['Texas', 'Alabama', 'Chicago', 'Memphis', 'Calif', 'Miami', 'Vegas'])
        self.assertTrue(len(self.dojo.offices) == 6, 'Cannot create more than 6 living spaces')
